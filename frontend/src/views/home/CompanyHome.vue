<template>
  <div class="page" v-auto-animate>
    ...
    <section class="grid grid-4 page-gap">
      <AppStatCard
    ...
    <section class="grid grid-3 page-gap">
      <AppPanel title="岗位数据趋势" desc="近 7 日收到简历数量变化" class="wide-card" :hover="false">
        <div class="bar-chart">
          <div v-for="item in companyTrend" :key="item.label" class="bar-item">
            <div class="bar-column" :style="{ height: `${Math.max(item.value * 10, 24)}px` }" />
      ...
      <AppAiCard
        title="AI 岗位描述生成"
        desc="输入岗位名称、薪资与技能要求，自动生成岗位职责和任职要求，减少 HR 编写岗位文案的时间。"
    ...
    <section class="grid grid-2 page-gap">
      <AppPanel title="候选人列表" desc="根据简历内容和岗位要求展示候选人优先级" :hover="false">
        ...
        <div class="candidate-list" v-auto-animate>
          <div v-for="item in candidates" :key="`${item.name}-${item.role}`" class="candidate-item">
            <div class="candidate-head">
              <div class="candidate-main">
                <el-avatar :size="42" class="candidate-avatar">{{ item.name.slice(0, 1) }}</el-avatar>
                <div>
                  <h3>{{ item.name }}</h3>
                  <div class="muted">{{ item.school }} · {{ item.role }}</div>
                </div>
              </div>
              <el-tag :type="item.match > 88 ? 'success' : item.match > 70 ? 'primary' : 'warning'" effect="plain">{{ item.status }}</el-tag>
            </div>
...
import { candidates, companyMetrics, companyTrend, recruitmentConversion } from '../../mock/dashboard'
...
const processRows = computed(() => recruitmentConversion.slice(1).map(item => ({
  label: item.label,
  value: item.value,
