  ...
  <div class="admin-page">
    <section class="admin-page__header">
      <div>
        ...
        <h1>企业信息管理</h1>
        <p>读取后端 <strong>GET /api/admin/companies</strong>，审核操作使用真实企业审核接口。</p>
      </div>
      <el-button :loading="loading" type="primary" @click="loadCompanies">刷新</el-button>
    </section>

    <section class="admin-panel filter-panel">
      <el-input v-model="filters.keyword" clearable placeholder="企业名称 / 行业 / 联系人" @keyup.enter="search" />
    ...
    <section v-if="errorMessage" class="state-panel state-panel--error">
      <div>
        <strong>企业列表加载失败</strong>
        <p>{{ errorMessage }}</p>
      </div>
      <el-button plain @click="loadCompanies">重试</el-button>
    </section>
      ...
      <el-table v-loading="loading" :data="rows" border stripe class="admin-table">
        ...
        <el-table-column label="企业" min-width="190">
          <template #default="{ row }">
            <strong class="primary-text">{{ row.companyName || '-' }}</strong>
            <p class="muted-text">{{ row.username || '-' }}</p>
          </template>
        </el-table-column>
        <el-table-column prop="industry" label="行业" min-width="130" />
        <el-table-column prop="companyScale" label="规模" min-width="120" />
    ...
    <el-dialog v-model="auditDialogVisible" title="企业审核" width="460px">
...
import { auditCompany, getAdminCompanies } from '../../api/admin.api'
...
async function loadCompanies() {
  ...
  try {
    ...
    rows.value = []
    total.value = 0
    errorMessage.value = error?.message || '请确认后端企业管理接口是否可用。'
  } finally {
    loading.value = false
  }
}
...
function reset() {
  Object.assign(filters, { keyword: '', auditStatus: '' })
  search()
}
...
async function submitAudit() {
  ...
  try {
    await auditCompany(currentRow.value.id, {
      auditStatus: auditForm.auditStatus,
      auditRemark: auditForm.auditRemark
    })
    ElMessage.success('企业审核已更新')
